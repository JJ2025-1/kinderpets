...
export async function GET(request: NextRequest) {
  try {
    ...
    const applications = await dbService.getApplications(shelterId);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const applicationId = await dbService.submitAdoptionApplication(Number(matchId), homeVisitDate);
